...
- 已知
  - 一系列的点$(\overrightarrow{x}_0, y_0), (\overrightarrow{x}_1, y_1), ..., (\overrightarrow{x}_n, y_n)$
  - 一个除了系数之外,剩下都已知的映射关系$f$, 也就是, $y = f(\overrightarrow{x}; \overrightarrow{k})$ 

- 求解
  - $\overrightarrow{k}$, 使得$f$能很好描述$\overrightarrow{x}$和$y$之间的关系